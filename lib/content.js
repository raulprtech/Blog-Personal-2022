...
function toTimelineItem(item, sourceType, defaults = {}) {
  ...
  return removeUndefined({
    ...
    organization: item.organization || item.event || item.venue || defaults.organization,
...
export async function getTimelineItems(lang = 'es') {
  const [trajectory, talks, projects, papers, ventures] = await Promise.all([
    getTrajectory(lang),
    getTalks(lang),
    getProjects(lang),
    getPapers(lang),
  ...
  return sortTimelineItems([
    ...trajectory.map((item) => toTimelineItem(item, 'trajectory', { category: item.category })),
    ...talks
